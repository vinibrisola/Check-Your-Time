...
export function* filterColaboradores(){
  ...
  try{
    ...
    if(res.colaboradores.length > 0) {
      yield put(updateColaborador({ colaborador: res.colaboradores[0],form: {...form, filtering: false, disabled: true }}));
    } else {
...
export function* addColaboradores(){
  ...
  try{
    ...
    let res = {};
    if(behavior === "create") {
      const response = yield call(api.post,`/colaborador`,{ salaoId: consts.salaoId, colaborador });
      res = response.data;
    } else {
